...
| `Fireshards` | Voorraad vuurscherven, het enige opraapbare verlies, de beloning per vijand en de vijandelijke hoopjes; kent de savewaarden maar geen UI |
| `FireShard` | Opgeslagen hoopje dat zijn eigen aantal nooit beheert: nabijheid, twee-traps absorptie en één signaal |
...
`Fireshards` is de enige eigenaar van de regels. `Fireshards.reward_for(enemy)` kiest de beloning: de per-instance `Damageable.fireshard_reward_override` wint van `EnemySettings.fireshard_reward`. Een verslagen `Damageable` met een beloning boven 0 vraagt om een hoopje op zijn sterfplek; de punten worden pas bijgeschreven wanneer de speler binnen `interaction_radius` komt en `FireShard.collect()` de absorptie start. Sterft de speler, dan verhuist de hele voorraad naar één `lost`-item op de sterfplek; een tweede dood verwijdert dat item voordat de nieuwe voorraad wordt vastgelegd. `PlayerCharacter` zendt `died` pas uit nadat de dode toestand is ingegaan, zodat het lichaam dat net viel zijn eigen verlies niet opzuigt.

Beide soorten items staan in `GameProgress.data.fire_shards` met een `area`-sleutel (`WorldArea.code` of het scenepad) en een positie als losse floats, zodat JSON-saves en oude saves blijven werken; de souls-era sleutels worden bij het laden omgezet. `Fireshards` spawnt per scenewissel precies de hoopjes van het geladen level en ruimt de rest op; de HUD leest alleen signalen en `pending_loss()`. Zie [FIRESHARDS.md](FIRESHARDS.md).
...
## Editor voor nieuwe levels

`addons/level_builder` schrijft complete scene-instances en Resources. De ondertab bevat uitsluitend de assetbibliotheek met opgeslagen native thumbnails; de rechterdock scheidt bouwen, contextueel bewerken en levelbeheer. Beide zijn begrensd door eigen scrollcontainers. Grote thumbnail- en meshgegevens zijn binaire `.res`-bestanden, geen ingebedde byte-arrays in leesbare catalogi.

`AreaSet` beschrijft gedeelde art/materialen/licht, `LevelAsset` een herbruikbare plaatsbare prefab en `WorldArea` de afzonderlijke gebiedsidentiteit. `authored_level.gd` hergebruikt PrototypeRoom/arena en leidt cameragrenzen af uit LevelTerrain.size. De speler, GameClock, navigatie, interacties, portals en saves blijven de bestaande gedeelde services gebruiken.

`LevelTerrain` bakt uitsluitend in de editor/offline. LevelPath levert een gefilterd padmasker, LevelTerrace en LevelRamp leveren exclusieve grondvoetafdrukken. Rampcurvepunten bevatten hun echte hoogte; cijfervelden en verticale gizmos bewerken dezelfde punten. Een automatisch aangesloten plateau kan zijn hoogte aan een rampeinde koppelen. Gegenereerde resources worden naast de scene in `<Levelnaam>.terrain/` opgeslagen. Bij invaliditeit blijft de laatst geldige geometrie behouden. Runtime laadt de opgeslagen mesh/collider.

Assetvoorbereiding bewaart expliciete modelcollision en maakt alleen bij ontbreken een gekozen vorm. Herimport vernieuwt uitsluitend GeneratedCollision; handmatige nodes en alle levelplaatsingen blijven behouden. EnemyPatrol vult per instantie EnemyBrain.authored_patrol in zonder EnemySettings te wijzigen. De editor geeft geplaatste enemies blijvende identiteiten; encounterdata verbindt bestaande health/portal/worldflag-contracten. Zie [LEVEL_BUILDER.md](LEVEL_BUILDER.md).

`SurfaceStyle` maakt grondmaterialen onafhankelijk van het gekozen assetpakket. Basisgrond, plateaus en ramps hebben optionele overrides. Een getekend vlak op hoogte nul gebruikt hetzelfde exclusieve oppervlaksysteem. De shader gebruikt een gemeenschappelijk fijn padmasker en aparte surfaces per materiaal. `RampConnection` sluit ramps loodrecht aan op een bestaande plateaurand, bewaakt de randbreedte en herstelt de verbinding bij bewerken. Een plateau dat vanuit een losse ramp gemaakt is kan zijn hoogte van het rampeinde lezen.

De terreinbakker filtert collineaire en dubbele driehoeken vóór mesh/collision-opslag. EnemySurfaceNavigation projecteert lokaal verticaal op de loopvloer; een Euclidisch dichtstbijzijnde punt verschuift op hellingen zijwaarts en mag daarom geen herstelbesturing veroorzaken. EnemyLocomotion vereenvoudigt alleen volledig gecontroleerde vrije corridors. Bij eerste grondcontact kan een geplaatste enemy zijn thuishoogte aan een inmiddels verlaagde vloer aanpassen; eigen patrouilles kennen geen limiet van zes seconden per punt.

De selectietool koppelt ray-hits op de zichtbare grond-/brug-/watergeometrie aan de authored nodes. De verplaatsing bewaart vooraf transforms en curves, inclusief grondvolgende verbindingen; afronden, annuleren en Undo/Redo herstellen die samen met de terreinsynchronisatie. Tijdelijke witte omtrekken blijven onowned. Water en bruggen melden transformwijzigingen aan de bakker en dragen selectie op hun gegenereerde kinderen vóór vervanging over aan hun eigen node.

`Strooi` deelt de plaatsingsroute en LevelAsset-scenes met `Plaats`, met penseelstraal, aantal en minimumafstand. Elke kandidaat volgt het oppervlak en controleert terreingrenzen, water, optioneel paden en bestaande objecten. `Overlap toestaan` negeert eerder geplaatste assets in de afstandscontrole; tussen objecten in `stroke_nodes` blijft de gekozen afstand gelden, ook over meerdere penseelstempels. De oppervlaktegrenzen blijven gelden. De plaatsingshoogte is een offset langs de verticale wereldas, toegepast vóór de afstandscontrole en ook bij wissen. Willekeurige draaiing is per strooisessie aan/uit te zetten zonder de LevelAsset te wijzigen. Een volledige plaatsings- of wisstreek is één editor-Undo-actie; de resulterende objecten zijn gewone opgeslagen scene-instances. De strooitool heeft geen runtimegenerator nodig.

Plateaus en trapeinden delen het bereik 0–100 m en de vaste stap van 0,5 m. Plaatsingsklikken gebruiken de gebakken zichtbare geometrie, inclusief verhoogde wateroppervlakken en bruggen. Een LevelWater bewaart de aangeklikte steunhoogte in `position.y`; zijn bodemhoogte is relatief aan dat niveau. `LevelTerrain.outlines()` knipt water op grond met dezelfde steunhoogte en verwijdert de bedekte droge stukken. Wateroppervlak en gebakken bodem gebruiken dezelfde exclusieve polygonen, zodat hogere eilanden en plateauranden in beide overeenkomen. Een terreinbak werkt de waterweergave bij na veranderingen aan de ondersteunende grond.
