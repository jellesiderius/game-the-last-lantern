...
for source in sorted((ROOT/'assets/environment').glob('forest_*/source.blend')):
 if (source.parent.name=='forest_ground' and only!='forest_ground') or (only and source.parent.name!=only):continue
 ...
 data=bpy.data.cameras.new('ReviewCamera');cam=bpy.data.objects.new('ReviewCamera',data);s.collection.objects.link(cam);s.camera=cam;data.type='ORTHO';data.ortho_scale=size;data.clip_end=size*30
 ...
 for name,off in [('front',(0,6,.7)),('left',(-6,0,.7)),('right',(6,0,.7)),('back',(0,-6,.7)),('game',(4,-4,6.7))]:
  cam.location=target+Vector(off)*size;cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler();s.render.filepath=str(out/(name+'.png'));bpy.context.view_layer.update();bpy.ops.render.render(write_still=True)
