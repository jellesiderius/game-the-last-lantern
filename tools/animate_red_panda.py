...
rig=bpy.data.objects['RedPanda_Rig'];c=bpy.data.collections['RedPanda_Character']
...
def pose(clip,t,duration):
 ...
 grip=V((.29,.12,.39));blade=V((.20,.90,-.38)).normalized()
 left_grip=V((-.31,.04,.36));left_dir=V((0,1,0));foot_phase=None;stride=.16;lift=.06;gait=0.;sway=0.
 ...
 if clip in ['walk','run']:
  ...
  bob=(-.040 if fast else -.017)-(.014 if fast else .010)*(1-math.cos(u*math.tau*2))*.5
  foot_phase=u;stride=.18 if fast else .12;lift=.065 if fast else .045
 ...
 if clip in ['heavy_charge','heavy_hold']:
  ...
  grip=grip.lerp(V((.30,-.075,.74)),a);blade=blade.lerp(V((.87,-.33,.35)).normalized(),a).normalized()
  left_grip=left_grip.lerp(V((-.17,.17,.55)),a)
 ...
 if clip in attacks:
  ...
  before=V((.32,-.035,.72 if heavy else .57));after=V((-.035,.27,.44 if heavy else .47))
  ...
  left_grip=left_grip.lerp(V((-.19,.15,.56)),weight)
 if clip=='dodge_roll':
  tuck=math.sin(math.pi*u)**.65;roll=-math.tau*ease(u);bob=-.07*tuck
  grip=grip.lerp(V((.20,.15,.55)),tuck);left_grip=left_grip.lerp(V((-.20,.15,.55)),tuck)
 ...
 if clip.startswith('bow_') and clip!='bow_empty':
  ...
  left_grip=left_grip.lerp(V((.0,.30,.53)),bow)
  grip=grip.lerp(V((.0,.24-.13*draw,.53)),bow)
 ...
 base=T((sway,0,bob))@pivot(RX(roll),V((0,0,.32)))
 body_transform=base@pivot(RZ(yaw)@RX(-lean),V((0,0,.32)))
 ...
 for sign,side in [(-1,'L'),(1,'R')]:
  ...
  if foot_phase is not None:
   ...
   if .32<cycle<.5:
    toe=ankle+V((0,.13,-.037));foot_matrix=pivot(RX(-.20*ease((cycle-.32)/.18)),toe)@foot_matrix
  ...
  if clip in ['dodge_roll','death']:
   ...
   ankle=body_transform@(ankle.lerp(V((sign*.14,.10,.22)),amount))
   foot_matrix=body_transform@rest['foot_'+side];foot_matrix.translation=ankle
  hip=body_transform@rest['leg_'+side].translation
  knee,ankle_clamped=joint(hip,ankle,lengths['leg_'+side],lengths['knee_'+side],body_transform@V((sign*.18,.25,.17)))
...
import json
(ROOT/'captures/red_panda/animation_floor_audit.json').write_text(json.dumps(floor_report,indent=2))
